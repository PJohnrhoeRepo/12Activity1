...
{	
	...
	if(empty($FirstName) || empty($LastName) || empty($Gender) || empty($Deparment) || empty($DateEmployed) || empty($Salary)) {	
		...
		//updating the table
		$sql = "UPDATE tbl_employees SET eFirstName=:eFirstName, eLastName=:eLastName, eGender=:eGender, eDepartment=:eDepartment, eDateEmployed=:eDateEmployed, eSalary=:eSalary WHERE eid=:id";
		...
		$query->bindparam(':eDepartment', $Deparment);
